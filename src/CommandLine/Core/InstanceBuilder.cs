    ...
    internal static class InstanceBuilder
    {
            ...
            CultureInfo parsingCulture)
        {
            ...
            if (arguments.Any())
            {
                ...
                {
                    return new NotParsed<T>(TypeInfo.Create(makeDefault().GetType()), preprocessorErrors);
                }
            }
            ...
            return allErrors.Any()
                ? (ParserResult<T>)new NotParsed<T>(TypeInfo.Create(instance.GetType()), allErrors)
